/**
 * FlowC Callback Handler
 * 
 * Receives HOLD/ROUTE_TO_REVIEW signals from FlowC when compliance checks flag invoices.
 * This endpoint is called by FlowC's flowbills-compliance-hook function when a transaction
 * is flagged for review.
 * 
 * Endpoint: POST /functions/v1/flowc-callback
 * 
...
interface FlowCCallbackPayload {
  invoice_id: string;
...
async function verifySignature(
  ...
  try {
    ...
    console.error('[FlowC Callback] Signature verification error:', error);
    return false;
  }
}
...
async function handleHold(
  ...
  if (updateError) {
    console.error('[FlowC Callback] Failed to update invoice status:', updateError);
    throw updateError;
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!signature || !webhookSecret) {
      console.error('[FlowC Callback] Missing signature or secret');
      return new Response(
    ...
    const payload: FlowCCallbackPayload = JSON.parse(rawBody);

    // Verify signature
    const isValid = await verifySignature(rawBody, signature, webhookSecret);
    if (!isValid) {
      console.error('[FlowC Callback] Invalid signature');
      return new Response(
    ...
    console.log(`[FlowC Callback] Processing ${payload.action} for invoice ${payload.invoice_id}`);
    ...
    switch (payload.action) {
      ...
      default:
        console.error(`[FlowC Callback] Unknown action: ${payload.action}`);
        return new Response(
    ...
    console.error('[FlowC Callback] Error:', error);
