...
// Types
export interface OilGasQueryRequest {
  query: string;
  user_id?: string;
  context?: any[];
}

export interface OilGasResponse {
  response: string;
  citations: string[];
...
// Core Logic Handler - Separation of Concerns
export async function processOilGasRequest(
  req: Request,
  ...
  try {
    ...
    const { query, user_id, context } = requestData;
    ...
    const industryContext = await retrieveOilGasContext(query, supabaseClient);
