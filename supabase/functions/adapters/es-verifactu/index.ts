...
async function generateLogbookEntry(
  ...
  try {
    ...
    const qrCode = `https://sede.agenciatributaria.gob.es/Sede/verificafactu/qr?${
      new URLSearchParams(qrData).toString()
    }`;
...
async function updateHashChain(
  invoiceData: any,
  previousHash?: string,
): Promise<VerifactuResponse> {
  try {
    ...
    if (previousHash && logbookResult.hashChain) {
      const isValidChain = await verifyHashChain(
        logbookResult.logbookEntry,
        previousHash,
        logbookResult.hashChain,
      );
