...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
  ...
  <cac:AccountingSupplierParty>
    <cac:Party>
      <cac:EndpointID schemeID="${invoiceData.supplier?.endpointScheme || '0088'}">${
    invoiceData.supplier?.endpointId || '1234567890123'
  }</cac:EndpointID>
      <cac:PartyIdentification>
        <cbc:ID schemeID="${invoiceData.supplier?.idScheme || '0088'}">${
    invoiceData.supplier?.id || '1234567890123'
  }</cbc:ID>
      </cac:PartyIdentification>
      <cac:PartyName>
      ...
      <cac:PostalAddress>
        <cbc:StreetName>${
    invoiceData.supplier?.address?.street || 'Sample Street 1'
  }</cbc:StreetName>
    ...
    invoiceData.supplier?.address?.country || 'DE'
  }</cbc:IdentificationCode>
        </cac:Country>
      </cac:PostalAddress>
      <cac:PartyTaxScheme>
  ...
  <cac:AccountingCustomerParty>
    <cac:Party>
      <cac:EndpointID schemeID="${invoiceData.customer?.endpointScheme || '0088'}">${
    invoiceData.customer?.endpointId || '9876543210987'
  }</cac:EndpointID>
      <cac:PartyIdentification>
        <cbc:ID schemeID="${invoiceData.customer?.idScheme || '0088'}">${
    invoiceData.customer?.id || '9876543210987'
  }</cbc:ID>
      </cac:PartyIdentification>
      <cac:PartyName>
      ...
      <cac:PostalAddress>
        <cbc:StreetName>${
    invoiceData.customer?.address?.street || 'Customer Street 1'
  }</cbc:StreetName>
        ...
        <cac:Country>
          <cbc:IdentificationCode>${
    invoiceData.customer?.address?.country || 'FR'
  }</cbc:IdentificationCode>
        </cac:Country>
      </cac:PostalAddress>
      <cac:PartyTaxScheme>
  ...
  <cac:TaxTotal>
    <cbc:TaxAmount currencyID="${invoiceData.currency || 'EUR'}">${
    invoiceData.taxAmount || '0.00'
  }</cbc:TaxAmount>
    <cac:TaxSubtotal>
      <cbc:TaxableAmount currencyID="${invoiceData.currency || 'EUR'}">${
    invoiceData.netAmount || '100.00'
  }</cbc:TaxableAmount>
      <cbc:TaxAmount currencyID="${invoiceData.currency || 'EUR'}">${
    invoiceData.taxAmount || '19.00'
  }</cbc:TaxAmount>
      <cac:TaxCategory>
        <cbc:ID>S</cbc:ID>
  ...
  <cac:LegalMonetaryTotal>
    <cbc:LineExtensionAmount currencyID="${invoiceData.currency || 'EUR'}">${
    invoiceData.netAmount || '100.00'
  }</cbc:LineExtensionAmount>
    <cbc:TaxExclusiveAmount currencyID="${invoiceData.currency || 'EUR'}">${
    invoiceData.netAmount || '100.00'
  }</cbc:TaxExclusiveAmount>
    <cbc:TaxInclusiveAmount currencyID="${invoiceData.currency || 'EUR'}">${
    invoiceData.totalAmount || '119.00'
  }</cbc:TaxInclusiveAmount>
    <cbc:PayableAmount currencyID="${invoiceData.currency || 'EUR'}">${
    invoiceData.totalAmount || '119.00'
  }</cbc:PayableAmount>
  </cac:LegalMonetaryTotal>
  ...
  <cac:InvoiceLine>
    <cbc:ID>1</cbc:ID>
    <cbc:InvoicedQuantity unitCode="${invoiceData.lines?.[0]?.unit || 'C62'}">${
    invoiceData.lines?.[0]?.quantity || '1'
  }</cbc:InvoicedQuantity>
    <cbc:LineExtensionAmount currencyID="${invoiceData.currency || 'EUR'}">${
    invoiceData.lines?.[0]?.amount || '100.00'
  }</cbc:LineExtensionAmount>
    <cac:Item>
    ...
    <cac:Price>
      <cbc:PriceAmount currencyID="${invoiceData.currency || 'EUR'}">${
    invoiceData.lines?.[0]?.unitPrice || '100.00'
  }</cbc:PriceAmount>
    </cac:Price>
  </cac:InvoiceLine>
</Invoice>`;
