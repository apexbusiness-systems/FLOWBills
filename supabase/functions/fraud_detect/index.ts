...
function extractBankInfo(
  xmlContent: string,
): { iban?: string; account_number?: string; bank_code?: string } {
  const bankInfo: any = {};

  // IBAN extraction
  const ibanMatch = xmlContent.match(
    /<cbc:ID[^>]*>([A-Z]{2}\d{2}[A-Z0-9]{4}\d{7}[A-Z0-9]{1,23})<\/cbc:ID>/,
  );
  if (ibanMatch) {
...
function extractTaxInfo(xmlContent: string): { vat_id?: string; tax_id?: string } {
  ...
  // VAT ID extraction
  const vatMatch = xmlContent.match(
...
async function checkAmountAnomaly(
  ...
  const amounts = historicalDocs.map((d: any) => parseFloat(d.total_amount)).filter((a: number) =>
    !isNaN(a)
  );
  ...
  const variance = amounts.reduce((sum: number, a: number) => sum + Math.pow(a - mean, 2), 0) /
    amounts.length;
  ...
  if (zScore > 3) {
    return {
      ...
      evidence: [
        `Invoice amount ${currentAmount} is ${
          zScore.toFixed(2)
        } standard deviations from historical mean of ${mean.toFixed(2)}`,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const overallRiskScore = fraudFlags.length > 0
      ? Math.max(...fraudFlags.map((f) => f.risk_score))
      : 0;
